...
public:
  ...
  anaTrgB(TString fileList) : anabase(fileList), _trg_conf_file("trg_setup.conf"), _NGB_rate_in_MHz(268.0), _fadc_electronic_noise_RMS(3.8082498), _name_ana_conf_file("./anaTrgB_p.conf")
  {
  }

  anaTrgB(TString file, Int_t key) : anabase(file, key), _trg_conf_file("trg_setup.conf"), _NGB_rate_in_MHz(268.0), _fadc_electronic_noise_RMS(3.8082498), _name_ana_conf_file("./anaTrgB_p.conf")
  {
  }
  ...
  static void npe_rate_calculations( TH1D *h1_rate_vs_npe, TH1D *h1_npe_w, Double_t tot_flux, Double_t tot_number_of_sim_events);
